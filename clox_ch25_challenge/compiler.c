...
static void initCompiler(Compiler* compiler, FunctionType type) {
    ...
    compiler->function = newFunction(); // allocate a new function object to compile into
    current = compiler;
    ...
    // the compiler’s locals array keeps track of which stack slots are associated with which local variables/temporaries
    // now, the compiler implicitly claims stack slot zero for the VM’s own internal use
...
static ObjFunction* endCompiler() {
    emitReturn();
    // previously, interpret() called into the compiler and passed in a Chunk to be written to
    // now, the compiler creates the function object itself
...
#ifdef DEBUG_PRINT_CODE
    if (!parser.hadError) {
        // check if the function’s name is NULL
...
// chew through arguments as long as there are commas after each expression
// once out, consume the final closing parenthesis and done
...
// for function calls
// already consumed the ( token, so then compile the arguments using argumentList() helper
// each argument expression generates code that leaves its value on the stack for the call
// after, emit a new OP_CALL instruction to invoke the function
...
// compile the function itself: its parameter list and block body
// helper function generates code that leaves the resulting function object on top of the stack
static void function(FunctionType type) {
    ...
    // function parameters
    // note the function’s arity by counting how many parameters we parse
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
    ...
    if (function->upvalueCount > 0) {
        ...
        // If no upvalues, just treat it like a normal constant
        emitBytes(OP_CONSTANT, makeConstant(OBJ_VAL(function)));
    }
}

// function declaration creates and stores a functions in a newly declared variable
// parse the name just like any other variable declaration
// function declaration at the top level will bind the function to a global variable
//      inside a block or other function, a function declaration creates a local variable
// it’s safe for a function to refer to its own name inside its body -> recursion (just can't call until completed)
...
static void forStatement() {
  ...
  if (match(TOKEN_VAR)) {
    ...
    varDeclaration();
    // get the slot of the loop variable
  ...
  if (!match(TOKEN_SEMICOLON)) {
    ...
    // get out of the loop if the condition is false
    exitJump = emitJump(OP_JUMP_IF_FALSE);
    emitByte(OP_POP);
  }
...
// return value expression is optional
// if no return value, the statement implicitly returns nil (OP_NILL)
// else, compile the return value expression and return it (OP_RETURN)
static void returnStatement() {
    // compile error to have a return statement outside of any function
