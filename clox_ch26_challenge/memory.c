...
/*
 * Cheney’s Algorithm: "stop-and-copy" collector that eliminates fragmentation by physically moving objects.
 *
 * The heap is split in half: From-Space and To-Space.
 * During collection, live objects are copied from From-Space to To-Space in a contiguous block.
 * When copying is complete, the entire From-Space is treated as empty, flip the spaces.
 */
static uint8_t* fromSpace;
static uint8_t* toSpace;
static size_t spaceSize = 1024 * 1024; // example size

static uint8_t* freePtr; // next available slot in To-Space
static uint8_t* scanPtr; // the next object to be inspected to see if it references other things that need to be copied over

void initHeap() {
    fromSpace = malloc(spaceSize); // the contiguous memory of fromSpace
    toSpace = malloc(spaceSize); // the contiguous memory of toSpace
    freePtr = toSpace; // setup for the first collection, points to the first slot of toSpace
    scanPtr = toSpace;
}

// helper function to calculate buffer size the object needs when copying it over
size_t sizeOfObject(Obj* object) {
    switch (object->type) {
        case OBJ_CLOSURE: {
            ObjClosure* closure = (ObjClosure*)object;
            // closures have a flexible array member (upvalues) at the end, so account for the array of pointers to upvalues
        ...
        case OBJ_STRING: {
            // a string is a variable-length object
            ObjString* string = (ObjString*)object;
            // sizeof(ObjString) is the header (containing type, isMarked, next) and string-specific metadata (length and hash)
            // string->length is the payload. ex. if the string is "apple", then 5 bytes
            // + 1 is for the null terminator (\0 - underlying C stuff)
...
// function to keep this object alive while the rest of the memory is destroyed
Obj* rescue(Obj* object) {
    if (object == NULL) return NULL;

    // check if the object is already in To-Space
    if (object->isMarked) {
        return (Obj*)object->next;
    }

    // copy the object to To-Space
    size_t size = sizeOfObject(object);
    Obj* destination = (Obj*)freePtr;
    memcpy(destination, object, size);
    freePtr += size;

    // mark the object as moved and store the To-Space location in the "next" pointer
    object->isMarked = true;
...
static Value rescueValue(Value value) {
    if (IS_OBJ(value)) {
        // rescue() returns an Obj*, so we wrap it back in a Value
        return OBJ_VAL(rescue(AS_OBJ(value)));
    }
    return value;
}

// function to scan an object in To-Space and update pointers from From-Space to To-Space
void scanObject(Obj* object) {
    switch (object->type) {
        case OBJ_CLOSURE: {
            ObjClosure* closure = (ObjClosure*)object;
            // update the pointer to rescue()
            closure->function = (ObjFunction*)rescue((Obj*)closure->function);
            // update the array of upvalue pointers
            for (int i = 0; i < closure->upvalueCount; i++) {
                closure->upvalues[i] = (ObjUpvalue*)rescue((Obj*)closure->upvalues[i]);
            }
            break;
        }
        case OBJ_FUNCTION: {
            ObjFunction* function = (ObjFunction*)object;
            // functions reference a string (their name)
            function->name = (ObjString*)rescue((Obj*)function->name);
            for (int i = 0; i < function->chunk.constants.count; i++) {
                if (IS_OBJ(function->chunk.constants.values[i])) {
                    // update the pointer in the constant table to the new location
                    AS_OBJ(function->chunk.constants.values[i]) =
                        rescue(AS_OBJ(function->chunk.constants.values[i]));
                }
            }
            break;
        }
        case OBJ_UPVALUE: {
            ...
            // if the upvalue is "closed", it points to its own value field.
            // if it is "open", it points to a location on the stack.
            // only rescue if upvalue is pointing to an object on the heap.
            if (IS_OBJ(*upvalue->location)) {
                *upvalue->location = rescueValue(*upvalue->location);
            }
            break;
        }
        // these objects are leaf nodes (not root) so they contain no pointers to other heap objects
        // nothing to update
        case OBJ_STRING:
        case OBJ_NATIVE:
            break;
...
void collectGarbage() {
    ...
    // reset To-Space pointers
    freePtr = toSpace;
    scanPtr = toSpace;

    // evacuate roots - locals
    for (Value* slot = vm.stack; slot < vm.stackTop; slot++) {
        if (IS_OBJ(*slot)) {
            AS_OBJ(*slot) = rescue(AS_OBJ(*slot));
        }
    }
    ...
    for (int i = 0; i < vm.globals.capacity; i++) {
        ...
        if (entry->key != NULL) {
            entry->key = (ObjString*)rescue((Obj*)entry->key);
            entry->value = rescueValue(entry->value);
        }
    }
...
void* reallocate(void* pointer, size_t oldSize, size_t newSize) {
    ...
    // if out of space in the current 'toSpace', trigger GC
    if (freePtr + newSize > toSpace + spaceSize) {
        collectGarbage();
    }
