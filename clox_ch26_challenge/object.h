...
/*
 * expected: 4 + 1 + 8 = 13 bytes
 * but, CPU requires pointers to be aligned to their size (8 bytes)
 * to ensure the next pointer starts on an 8-byte boundary, the compiler inserts 3 bytes of padding after isMarked
 * Total size: 16 bytes
 */
struct Obj {
    ObjType type;     // 4 bytes - enum
    bool isMarked;    // 1 byte
    struct Obj* next; // 8 bytes - pointer // avoids leaking memory
